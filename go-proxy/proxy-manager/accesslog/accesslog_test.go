...
func (m *mockDB) LogAccessRequest(entry AccessLogEntry) error { atomic.AddInt64(&m.calls, 1); return nil }
func (m *mockDB) GetRecentRequests(limit int) ([]AccessLogEntry, error) { return nil, nil }
func (m *mockDB) GetRequestsByRoute(route string, limit int) ([]AccessLogEntry, error) { return nil, nil }
...
func TestLoggerBasic(t *testing.T) {
    ...
    foundErr := false
    for _, e := range errs { if e.Status >= 400 { foundErr = true; break } }
    if !foundErr { t.Fatalf("expected at least one error entry") }
    ...
    if !l.IsEnabled() { t.Fatalf("logger should be enabled") }
    l.Disable()
    if l.IsEnabled() { t.Fatalf("logger should be disabled") }
    l.Enable()
    l.Clear()
}
