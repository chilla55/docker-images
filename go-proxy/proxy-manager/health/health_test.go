...
func (m *mockDB) RecordHealthCheck(service, url string, success bool, duration time.Duration, statusCode int, err string) error { m.called++; return nil }
func (m *mockDB) GetHealthCheckHistory(service string, limit int) ([]HealthCheckResult, error) { return nil, nil }
...
func TestCheckerCheckSuccessAndFailure(t *testing.T) {
    ...
    if st := c.calculateStatus(0, 0); st != StatusUnknown { t.Fatalf("expected unknown") }
    if st := c.calculateStatus(9, 10); st != StatusHealthy { t.Fatalf("expected healthy") }
    if st := c.calculateStatus(5, 10); st != StatusDegraded { t.Fatalf("expected degraded") }
    if st := c.calculateStatus(4, 10); st != StatusDown { t.Fatalf("expected down") }
...
func TestGetStatusErrorsAndUnhealthyList(t *testing.T) {
    ...
    if len(unhealthy) == 0 { t.Fatalf("expected at least one unhealthy service") }
}
