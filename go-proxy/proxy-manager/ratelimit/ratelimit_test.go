...
func TestAllowMinuteLimit(t *testing.T) {
	...
	if allowed {
		t.Fatal("third request should be blocked by minute limit")
	}
	if reason == "" {
...
func TestAllowHourlyLimit(t *testing.T) {
	...
	allowed, _ := l.Allow("5.6.7.8", "/hour")
	if allowed {
		t.Fatal("4th request should be blocked by hourly limit")
	}
}
...
func TestMiddlewareWithXFF(t *testing.T) {
	...
	if hits != 2 {
		t.Fatalf("next handler should be called twice, got %d", hits)
	}
}
...
func TestWhitelistCIDR(t *testing.T) {
	...
	l := NewLimiter(cfg, &mockDB{})
	if err := l.AddWhitelist("203.0.113.0/24"); err != nil {
		t.Fatal(err)
	}
	...
	for i := 0; i < 5; i++ {
		...
		mw.ServeHTTP(rec, req)
		if rec.Code != http.StatusOK {
			t.Fatalf("whitelisted request expected 200, got %d", rec.Code)
		}
	}
	if hits != 5 {
		t.Fatalf("expected 5 passes to next, got %d", hits)
	}
}
...
func TestGetStats(t *testing.T) {
	...
	if stats["active_windows"].(int) == 0 {
		t.Error("expected active windows > 0")
	}
}
