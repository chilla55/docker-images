...
type hijackRW struct{ http.ResponseWriter; conn *dummyConn }
func (h *hijackRW) Hijack() (net.Conn, *bufio.ReadWriter, error) { h.conn = &dummyConn{}; return h.conn, bufio.NewReadWriter(bufio.NewReader(nil), bufio.NewWriter(nil)), nil }
...
func TestAddRemoveRouteAndFind(t *testing.T) {
    ...
    // find backend exact
    if b := s.findBackend("example.com", "/api"); b == nil { t.Fatalf("backend not found") }
    // find route for headers
    r := s.findRoute("example.com", "/api")
    rr := httptest.NewRecorder()
    s.applyHeaders(rr, r)
    if rr.Header().Get("X-Test") != "1" { t.Fatalf("route header not applied") }

    // Remove
    s.RemoveRoute([]string{"example.com"}, "/api")
    if s.findBackend("example.com", "/api") != nil { t.Fatalf("backend should be removed") }
}

func TestWildcardAndCertSelection(t *testing.T) {
    s := NewServer(Config{})
    // wildcard matching
    if !s.matchWildcard("*.example.com", "sub.example.com") { t.Fatalf("should match one-level subdomain") }
    if s.matchWildcard("*.example.com", "deep.sub.example.com") { t.Fatalf("should not match deep subdomains") }

    // certificate selection
    cert := tls.Certificate{}
    s.UpdateCertificates([]CertMapping{{Domains: []string{"example.com"}, Cert: cert}})
    if _, err := s.getCertificate(&tls.ClientHelloInfo{ServerName: "example.com"}); err != nil { t.Fatalf("getCertificate error: %v", err) }
}
...
func TestBlackholeCounts(t *testing.T) {
    ...
    s.ServeHTTP(rw, req)
    if s.GetBlackholeCount() != 1 { t.Fatalf("expected blackhole count=1") }
}
