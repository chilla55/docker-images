...
func TestOpenAndBasicOps(t *testing.T) {
    ...
    if _, err := db.GetRecentRequests(10); err != nil { t.Fatalf("GetRecentRequests failed: %v", err) }
    if _, err := db.GetRequestsByRoute("/", 10); err != nil { t.Fatalf("GetRequestsByRoute failed: %v", err) }
    if _, err := db.GetErrorRequests(10); err != nil { t.Fatalf("GetErrorRequests failed: %v", err) }

    // Health check record/query should succeed with test table
    if err := db.RecordHealthCheck("svc", "http://localhost", true, 0, 200, ""); err != nil { t.Fatalf("RecordHealthCheck failed: %v", err) }
    if _, err := db.GetHealthCheckHistory("svc", 10); err != nil { t.Fatalf("GetHealthCheckHistory failed: %v", err) }
    ...
    if err := db.CleanupSecurityLogs(90, "*"); err != nil { t.Fatalf("CleanupSecurityLogs failed: %v", err) }
    if err := db.CleanupAuditLogs(90); err != nil { t.Fatalf("CleanupAuditLogs failed: %v", err) }
    if err := db.CleanupMetrics(90, "*"); err != nil { t.Fatalf("CleanupMetrics failed: %v", err) }

    // GetAuditLogs with test table
    if _, err := db.GetAuditLogs(10, "", "", time.Time{}); err != nil { t.Fatalf("GetAuditLogs failed: %v", err) }
