#!/bin/bash
# ============================================================================
# Environment Validator for Pelican Panel Deployment
# ============================================================================
...
print_header "Pelican Panel Deployment Validator"
...
REQUIRED_NETWORKS=("web-net" "mariadb-net" "redis-net")
...
# Check go-proxy
if docker service ls 2>/dev/null | grep -q "proxy\|go-proxy"; then
    check_pass "go-proxy service found"
else
    check_warn "go-proxy service not found in Swarm"
fi
...
for script in "healthcheck.sh"; do
    if [ -x "$script" ]; then
...
if [ -f "docker-compose.swarm.yml" ]; then
    ...
    if grep -q "APP_URL: https://panel.chilla55.de" docker-compose.swarm.yml; then
        check_pass "APP_URL configured"
    else
        check_warn "APP_URL may need updating in docker-compose.swarm.yml"
    fi
...
if [ $FAILED -eq 0 ]; then
    ...
    echo -e "  3. Build: ${BLUE}make build${NC}"
    echo -e "  4. Deploy: ${BLUE}docker stack deploy -c docker-compose.swarm.yml pelican${NC}"
    echo -e "  5. Monitor: ${BLUE}docker service logs -f pelican_php-fpm${NC}"
    exit 0
else
    echo -e "${RED}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
